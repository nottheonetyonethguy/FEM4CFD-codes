...
for loadstep = 1:maxsteps
		...
		for elnum = 1:nelem
				...
				[Klocal, Mlocal, K_supg, M_supg, K_shock] = stiffnessAndForce_quad4_euler2D(elem_node_conn(elnum, :), node_coords, elemData, matData, U, dt, soln_initial, soln_full_prev);
				
				% get the indices for assembly
				inds4Assy = elem_dof_conn(elnum, :);
				
				% assemble
				Kglobal(inds4Assy, inds4Assy) = Kglobal(inds4Assy, inds4Assy) + Klocal + K_supg + K_shock;
...
plot_results(u, gamma, elem_node_conn, node_coords);
