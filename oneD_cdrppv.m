...
for iter = 1:9
    ...
    for elnum = 1:nelem
        ...
        [Klocal, Flocal] = galerkinApproximation(c, mu, he, nGP, gpts, gwts, elem_dofs, node_coords, soln_full, Klocal, Flocal);
        ...
        [Klocal, Flocal] = supg(c, mu, he, alpha, tau, nGP, gpts, gwts, elem_dofs, node_coords, soln_full_supg, Klocal, Flocal);
        ...
        [Klocal, Flocal] = ppv(c, mu, he, alpha, tau, nGP, gpts, gwts, elem_dofs, node_coords, soln_full_ppv, Klocal, Flocal);
...
function [Klocal_g, Flocal_g] = galerkinApproximation(a, mu, h, nGP, gpts, gwts, elem_dofs, node_coords, soln_full, Klocal, Flocal)
    Klocal_g = zeros(2, 2);
    Flocal_g = zeros(2, 1);
...
function [Klocal_supg, Flocal_supg] = supg(a, mu, h, alpha, tau, nGP, gpts, gwts, elem_dofs, node_coords, soln_full_supg, Klocal, Flocal)
...
function [Klocal_ppv, Flocal_ppv] = ppv(a, mu, h, alpha, tau, nGP, gpts, gwts, elem_dofs, node_coords, soln_full_ppv, Klocal, Flocal)
    ...
    for gp = 1:nGP
        ...
        kadd = max(0.0, (a - tau * a * f + tau * a * abs(f)) * h / 2 - (mu + tau * a * a) + (f + tau * f * abs(f)) * h * h / 6);
